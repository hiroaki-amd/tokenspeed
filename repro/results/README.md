...
Each file is named for the stage that writes it, so your own `scripts/all.sh`
overwrites these in place and `git diff` is the comparison against our numbers.
The stage 2 files were `ruler_speed_ctx32768.*` until the context length stopped
varying; they are `ruler_speed.*` now, matching what the script writes.

Which kernel each file is from, as of the partial rerun on `7d742874`:

```
sparsity_sweep            7d742874, current
ruler_speed               7d742874, current
...
ruler_speed.log / .json              stage 2, all 13 tasks, ctx 32768, th 0.03
...
Both `ruler_speed` and `ruler_speed_quick` were regenerated after
...
be isolated from each other in the first place. `ruler_speed` no
